...
function sub_navbar($node) {
	...
	if (CONST_FLAG_PROFILER) {
		$html_right 	.= "<div class=\"color-gray\">[".$d13->profile->profile_get().' '. misc::getlang("ms"). "]</div>&nbsp;";
	}
	
	//- - - Place Admin Panel
	if ((isset($_SESSION[CONST_PREFIX.'User']['level'])) && ($_SESSION[CONST_PREFIX.'User']['level'] >= 3)) {
		$html_right 	.= "<a class=\"external\" href=\"".CONST_BASE_PATH."index.php?p=admin\">".misc::getlang("adminPanel")."</a>";
	}
	...
	foreach ($game['navigation'] as $nav) {
		if ($nav['active']) {
			if ( ($nav['login'] && isset($_SESSION[CONST_PREFIX.'User']['id'])) ||(!$nav['login'] && !isset($_SESSION[CONST_PREFIX.'User']['id'])) ) {
				$pass = true;
				$nodeId = 0;
				$html = '<a class="tooltip-bottom link external" data-tooltip="'.misc::getlang($nav['name']).'" href="index.php?p='.$nav['link'].'&nodeId='.$nodeId.'"><span><img class="resource" src="'.CONST_DIRECTORY.'templates/'.$_SESSION[CONST_PREFIX.'User']['template'].'/images/icon/'.$nav['icon'].'.png"></span></a>';
				if ($nav['class'] == 'left') {
					$html_left .= $html;
				} else {
					$html_right .= $html;
