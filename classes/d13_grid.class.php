...
class grid
{
 ...
 public function get($x, $y) {
  global $d13;
  $result=$d13->db->query('select * from grid where (y between '.($y-3).' and '.($y+3).')  and (x between '.($x-3).' and '.($x+3).') order by y desc, x asc');
  for ($i=0; $row=$d13->db->fetch($result); $i++) $this->data[$i]=$row;
 }
 ...
 public function getAll() {
  global $d13;
  $result=$d13->db->query('select * from grid order by y desc, x asc');
  for ($i=0; $row=$d13->db->fetch($result); $i++) $this->data[$i]=$row;
 }
 ...
 {
  global $d13;
  $result=$d13->db->query('select * from grid where x="'.$x.'" and y="'.$y.'"');
  $sector=$d13->db->fetch($result);
  return $sector;
 }
